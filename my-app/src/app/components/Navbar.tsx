...
import { FiCodepen } from "react-icons/fi"; // Updated icon
...
const Navbar = () => {
  ...
  const [balance, setBalance] = useState<string>('0 KV'); // Set initial balance to '0 BW'
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        // Fetch the balance here (for example purposes, set to 15 BW)
        setBalance('15 KV'); // Replace this with your logic to fetch balance
      } catch (error) {
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 flex justify-between items-center p-6 bg-transparent z-50">
      ...
      >
        {/* <FiCodepen className="mr-2 text-yellow-300 text-2xl animate-bounce" /> */}
